...
<section class="premium-page page-grid">
  ...
  <section #printArea class="panel full report-print" *ngIf="report; else loadingTpl"><div class="section-head"><div><h2>Financial summary</h2><p>Generated {{ report.generatedAt | date:'medium' }}</p></div><span class="badge success">{{ report.savingsRate | number:'1.0-1' }}% savings</span></div><div class="quick-grid"><div class="quick-action"><strong>{{ report.income | currency:'INR' }}</strong><small>Income</small></div><div class="quick-action"><strong>{{ report.expenses | currency:'INR' }}</strong><small>Expenses</small></div><div class="quick-action"><strong>{{ report.balance | currency:'INR' }}</strong><small>Balance</small></div></div><h3>Category report</h3><div class="category-row" *ngFor="let item of report.categories"><span>{{ item.category }}</span><strong>{{ item.total | currency:'INR' }}</strong></div><h3>AI summary</h3><div class="suggestion" *ngFor="let item of report.insights">{{ item }}</div></section>
